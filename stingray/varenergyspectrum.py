...
class VarEnergySpectrum(metaclass=ABCMeta):
    ...
    def to_astropy_table(self):
        ...
        -------
        astropy.table.Table
            A table containing the spectrum data with columns for energy,
            spectrum values, and errors.
        """
        data = {
            'energy': self.energy,
            'spectrum': self.spectrum,
            'spectrum_error': self.spectrum_error,
            'energy_intervals_low': np.array(self.energy_intervals)[:, 0],
            'energy_intervals_high': np.array(self.energy_intervals)[:, 1]
        }
        
        table = Table(data)
        
        # Store metadata
        table.meta['freq_interval'] = self.freq_interval
        table.meta['use_pi'] = self.use_pi
        table.meta['bin_time'] = self.bin_time
        if hasattr(self, 'ref_band'):
            table.meta['ref_band'] = self.ref_band.tolist()
        if hasattr(self, 'segment_size') and self.segment_size is not None:
            table.meta['segment_size'] = self.segment_size
        if hasattr(self, 'delta_nu') and self.delta_nu is not None:
            table.meta['delta_nu'] = self.delta_nu
        if hasattr(self, 'return_complex'):
            table.meta['return_complex'] = self.return_complex
        if hasattr(self, 'same_events'):
            table.meta['same_events'] = self.same_events
            
        # Store class-specific attributes
        if hasattr(self, 'norm'):
            table.meta['norm'] = self.norm
        if hasattr(self, 'normalization'):
            table.meta['normalization'] = self.normalization  
        if hasattr(self, 'spectral_type'):
            table.meta['spectral_type'] = self.spectral_type
            
        return table

    @classmethod
    def from_astropy_table(cls, table, events=None, events2=None):
        """
        Create a spectrum object from an Astropy Table.

        Parameters
        ----------
        table : astropy.table.Table
            The table containing spectrum data
        events : EventList, optional
            Event list (required for some spectrum types)
        events2 : EventList, optional  
            Second event list if different from events

        Returns
        -------
        spectrum : VarEnergySpectrum subclass
            A new spectrum object with the data from the table
        """
        # Reconstruct energy_intervals from the table
        energy_intervals = list(zip(
            table['energy_intervals_low'], 
            table['energy_intervals_high']
        ))
        
        # Get metadata
        freq_interval = table.meta.get('freq_interval', [0, 1])
        use_pi = table.meta.get('use_pi', False)
        bin_time = table.meta.get('bin_time', 1)
        ref_band = table.meta.get('ref_band')
        if ref_band is not None:
            ref_band = np.array(ref_band)
        segment_size = table.meta.get('segment_size')
        return_complex = table.meta.get('return_complex', False)
        
        # Create a minimal events object if not provided
        if events is None:
            from stingray.events import EventList
            events = EventList([0], energy=[1])
            
        # Create instance - this is a placeholder since this is an abstract class
        # Subclasses should override this method
        instance = cls.__new__(cls)
        instance.energy_intervals = energy_intervals
        instance.freq_interval = freq_interval
        instance.use_pi = use_pi
        instance.bin_time = bin_time
        instance.ref_band = ref_band
        instance.segment_size = segment_size
        instance.delta_nu = table.meta.get('delta_nu')
        instance.return_complex = return_complex
        instance.same_events = table.meta.get('same_events', True)
        
        # Set spectrum data
        instance.spectrum = np.array(table['spectrum'])
        instance.spectrum_error = np.array(table['spectrum_error'])
        
        # Set class-specific attributes
        if 'norm' in table.meta:
            instance.norm = table.meta['norm']
        if 'normalization' in table.meta:
            instance.normalization = table.meta['normalization']
            
        return instance


class AveragedXYSpectrum(VarEnergySpectrum):
    """
    Generic averaged cross-spectral variability spectrum between two energy bands.
    
    This class computes averaged cross-spectral quantities (like covariance, 
    coherence, lag) between energy bands X and Y as a function of energy.
    
    Parameters
    ----------
    events : :class:`stingray.events.EventList` object
        event list for the first energy band (X)
    ...
    ----------------
    ref_band : ``[emin, emax]``, floats; default ``None``
        minimum and maximum energy of the reference band (Y). If ``None``, the
        full band is used.
    ...
    events2 : :class:`stingray.events.EventList` object
        event list for the second energy band (Y), if different from events
        
    spectral_type : str, default 'covariance'
        Type of spectral quantity to compute ('covariance', 'lag', 'coherence')
        
    Attributes
    ----------
    events1 : array-like
        list of events used for the X band
        
    events2 : array-like  
        list of events used for the Y band
    ...
    spectral_type : str
        The type of spectral quantity computed
    """
    ...
    def _spectrum_function(self):
        """
        Compute the averaged cross-spectral quantity for each energy interval.
        
        This is a generic implementation that computes covariance by default.
        """
        # Extract events from the reference band
        ...
        for i, eint in enumerate(show_progress(self.energy_intervals)):
            ...
            # Calculate cross spectrum between subject and reference bands
            ...
            if results_cross is None:
                continue
                
            cross = results_cross["power"]
            
            # Get frequency mask for integration
            good = self._get_good_frequency_bins()
            
            if self.spectral_type == 'covariance':
                # Compute covariance (absolute value of cross spectrum)
                Cmean = np.mean(np.abs(cross[good]))
                # Simple error estimate - should be improved for production use
                Cmean_err = np.std(np.abs(cross[good])) / np.sqrt(np.sum(good))
            elif self.spectral_type == 'lag':
                # Compute phase lag
                freq = results_cross["freq"] 
                f_center = (self.freq_interval[0] + self.freq_interval[1]) / 2
                Cmean = np.mean(np.angle(cross[good]) / (2 * np.pi * freq[good]))  
                Cmean_err = np.std(np.angle(cross[good]) / (2 * np.pi * freq[good])) / np.sqrt(np.sum(good))
            else:
                # Default to covariance magnitude
                Cmean = np.mean(np.abs(cross[good]))
                Cmean_err = np.std(np.abs(cross[good])) / np.sqrt(np.sum(good))
            
            self.spectrum[i] = Cmean
            self.spectrum_error[i] = Cmean_err

    @classmethod  
    def from_astropy_table(cls, table, events=None, events2=None):
        """
        Create an AveragedXYSpectrum from an Astropy Table.
        ...
        table : astropy.table.Table
            The table containing spectrum data
        events : EventList, optional
            Event list for X band (required for reconstruction)
        events2 : EventList, optional
            Event list for Y band if different from events

        Returns
        -------
        spectrum : AveragedXYSpectrum
            A new AveragedXYSpectrum object with the data from the table
        """
        # Reconstruct energy_spec from the table
        energy_intervals_low = table['energy_intervals_low']
        energy_intervals_high = table['energy_intervals_high'] 
        energy_spec = np.concatenate([energy_intervals_low, [energy_intervals_high[-1]]])
        ...
        ref_band = table.meta.get('ref_band')
        segment_size = table.meta.get('segment_size')
        spectral_type = table.meta.get('spectral_type', 'covariance')
        
        # Create a minimal events object if not provided
        if events is None:
            from stingray.events import EventList
            events = EventList([0, bin_time], energy=[1, 1])
        ...
        # Override the computed spectrum with the table data
        instance.spectrum = np.array(table['spectrum'])
        instance.spectrum_error = np.array(table['spectrum_error'])
...
class RmsSpectrum(VarEnergySpectrum):
    ...
    def from_astropy_table(cls, table, events=None, events2=None):
        """
        Create an RmsSpectrum from an Astropy Table.
        ...
        table : astropy.table.Table
            The table containing spectrum data
        events : EventList, optional
            Event list (required for reconstruction)
        events2 : EventList, optional  
            Second event list if different from events
        ...
        spectrum : RmsSpectrum
            A new RmsSpectrum object with the data from the table
        """
        # Reconstruct energy_spec from the table
        energy_intervals_low = table['energy_intervals_low']
        energy_intervals_high = table['energy_intervals_high']
